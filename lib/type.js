		...
		toString: function()
		{
			if (spec.format)
			{
				...
				if (spec.format.__template)
				{
					...
					return spec.format.apply(this, attrNames.map(function(n)
					{ 
						return String(get(that, n)); 
					}));
				}
			...
			return String(get(this, attrNames[0]));
		}
	});
	...
	return type(name,
	{
		constructor: function()
		{
			...
			for (i in types)
			{
				try
				{
					return types[i].apply(null, arguments);
				}
				catch(_) {}
			}
...
function literal(value)
{
	return function(val)
	{
		...
		throw new TypeError("Must be " + val);
	};
}
