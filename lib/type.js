...
function type(name, spec)
{
	...
	function ctor()
	{
		...
		if (specLength === 1 && (
			arg === null ||
			((t = typeof arg) !== 'object') ||
			(t === 'object' && !arg.hasOwnProperty(attrNames[0])) 
		))
		{
			var tmp = {};
			tmp[attrNames[0]] = arg;
	...
	ctor.prototype.inspect = function()
    {
        ...
        return name + '(' + attrNames.map(function(n) { return util.inspect(get(that, n) || undefined); }).join(', ') + ')';
    };
...
function collection(name, childType, surrounds, delim)
{
    ...
    .inspect(function()
    {
        return this.constructor.__type__ + '(' + 
            this.__vals.map(function(v) { return v.inspect(); }).join(', ') + 
            ')';
    })
    .format(function()
    {
